...
function getDigitalRoot(num) {
  ...
  const sum = array.reduce((s, item) => s + Number(item), 0);
  if (sum > 9) {
    const finalSum = String(sum).split('').reduce((s, item) => s + Number(item), 0);
    return finalSum;
  }
  return sum;
}
...
function isBracketsBalanced(str) {
  ...
  for (let i = brackets.length - 1; i >= 0; i -= 1) {
    if (str.includes(brackets[i])) {
      const newStr = str.replace(brackets[i], '');
...
function getCommonDirectoryPath(pathes) {
  ...
  pathes[0].split('').some((item, index) => {
    if (pathes.every((elem) => elem[index] === item)) {
      equalDir = `${equalDir}${item}`;
      return false;
    }
    return true;
  }, '');
  return equalDir.slice(0, equalDir.lastIndexOf('/') + 1);
}
